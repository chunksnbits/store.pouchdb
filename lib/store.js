...
PouchDbStore.prototype.store = function (items) {
  ...
  return this._adapter.store(convertedItems)
    .then(function (result) {
      var updatedItems = _.map(result, function (data, index) {
...
PouchDbStore.prototype._convertToItem = function (originalItems, storedData) {
  ...
  function convert (original, storedData) {
    ...
    if (original && original instanceof Item) {
      return Item.reapplyPrototype(self, _.extend(original, storedData));
    }
    else {
...
PouchDbStore.prototype._convertToSimpleObject = function (items) {
  ...
  function omit (item, omittables) {
    _.each(omittables, function (omittable) {
      delete item[omittable];
    });
  ...
  function clean (item) {
    ...
    if (processed[item]) {
      return omit(item, _.union(['_store', '__store'], _.functions(item)));
    }
    ...
    omit(item, _.union(['_store', '__store'], _.functions(item)));
