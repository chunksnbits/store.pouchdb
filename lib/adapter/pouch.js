...
var EventHandler = require('../core/event-handler');
...
// ------------------------------------------------------------ Object creation
// ------------------------------------------------------- Constructor
function PouchAdapter (collection, pouch) {
...
PouchAdapter.prototype._storeItemRelations = function (item) {
  ...
  // console.log('_storeItemRelations', this.collection._schema);

  var schema = this.collection._schema;
  var relations = {};
...
PouchAdapter.prototype._addItemRelations = function (items) {
  ...
  function fetchHasManyRelations (item) {
    return _.map(self.collection._schema.hasMany, function (connection, name) {
  ...
  function fetchHasOneRelations (item) {
    return _.map(self.collection._schema.hasOne, function (connection, name) {
...
PouchAdapter.prototype.remove = function (query) {
  var isArray = _.isArray(query);
  var isCollection = query.toString() === '[object ModelInstance]';
...
PouchAdapter.prototype._extendWithMetaInformation = function (items) {
  ...
  _.each(items, function (item) {
    item.$info = _.merge({
      ...
      updatedAt: now,
      collection: self.collection._name
    });

    if (self.schema) {
      item.$info.schema = self.schema;
    }
  });
