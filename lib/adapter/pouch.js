...
PouchAdapter.prototype.store = function (items) {
  ...
  return q.all(promises)
    ...
    .then(function (itemsToStore) {
      //
      // 8. Convert to format processible by PouchDb, i.e.,
      //    remove meta information like '_schema' and functions.
      //
      // console.log('8.', itemsToStore);
      return self._convertToSimpleObject(itemsToStore);
    })
    .then(function (itemsToStore) {
      //
      // 9. Store changed items.
      ...
      //
      // console.log('9.', itemsToStore);
      return self.pouch.bulkDocs(self._convertToPouch(itemsToStore));
    })
    .then(function (response)  {
      //
      // 10. Some errors are not thrown by pouch but
    ...
    .then(function (responses)  {
      //
      // 11. Merge recently updated and unchanged items
      //    back together to allow for a consistent response.
      //
      // console.log('11.', responses, markedItems);
      var index = 0;
    ...
    .then(function (updated) {
      //
      // 12. Reassemble items, their relations and the updated
    ...
    .then(function (result) {
      //
      // 13. For debugging purposes: Allows to print the final
      //     result.
      //
    ...
    .catch(function (error) {
      //
      // E. Capture any exception that pops up
    ...
    if (item.id) {
      return self._findOne(item.id)
        .catch(function () {
          ...
          // In case item holds an id, but has not been
    ...
    .then(function (storedItems) {
      return _.map(storedItems, function (storedItem, index) {
        ...
        if (storedItem) {
          ...
          isChanged = !deepEqual(storedItem, self._convertToSimpleObject(item));
        }

        // console.log('markChanged', isChanged);
...
PouchAdapter.prototype._storeItemRelations = function (item) {
  ...
  function storeHasManyRelation (item) {
    return _.map(schema.hasMany, function (connection, name) {
      ...
      relatedItems = _.filter(relatedItems, function (relatedItem) {
        // Skip circular data
        return !relatedItem.__marked;
      });
  ...
  function storeHasOneRelation (item) {
    return _.map(schema.hasOne, function (connection, name) {
      ...
      // Stop when encountering circular data
      if (value.__marked) {
        return;
      }
  ...
  // Mark items as beeing processed to
  // prevent circular data from being processed multiple
  // times
  item.__marked = true;
  ...
  return q.all(promises)
    .then(function () {
      // Remove processing marker again before storing item
...
PouchAdapter.prototype._removeAll = function () {
  var self = this;
...
PouchAdapter.prototype._convertToSimpleObject = function (items) {
