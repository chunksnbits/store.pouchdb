...
var Adapter = require('./shelfdb-adapter.js');
var Server = require('./shelfdb-server.js');
...
ShelfDb.load = function (collectionName, options) {
  ...
  // Only provide server adapter in case sync is required.
  //
  if (ShelfDb.Server.stopped() && options.sync !== false) {
    ShelfDb.Server.listen();
  }
