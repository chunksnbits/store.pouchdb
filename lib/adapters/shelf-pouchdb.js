...
PouchAdapter.prototype._storeItemRelations = function (item) {
  ...
  function extractIds (docs) {
    return _.map(docs, function (docs) {
      return item.id;
    });
  }
...
function _formatError (method, type, message, error) {
  return new CollectionError(method, type, message, error);
}
