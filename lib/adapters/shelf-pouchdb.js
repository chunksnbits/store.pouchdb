...
// // ------------------------------------------------------------ Inserts
// // ------------------------------------------------------- store
PouchAdapter.prototype.store = function () {

  var self = this;

  var items = arguments.length === 1 ? _.first(arguments) : _.toArray(arguments);
...
PouchAdapter.prototype.on = function (event, callback) {
  var registrationPointer = this.pouch.changes({
    since: 'now',
    live: true
  }).on(event, callback);

  var listener = new EventListener(registrationPointer);
...
// ------------------------------------------------------------ Listener
function EventListener (registrationPointer) {
  this.registrationPointer = registrationPointer;
}

EventListener.prototype.off = function () {
  this.registrationPointer.cancel();
};
